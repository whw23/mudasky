...
class TestSetDisciplines:
    """POST /universities/list/detail/disciplines 端点测试。"""
    ...
    async def test_set_disciplines_success(self, client, superuser_headers):
        """设置学科关联成功返回 204。"""
        self.mock_svc.set_disciplines.return_value = None

        resp = await client.post(
            "/admin/web-settings/universities/list/detail/disciplines",
            json={
                "university_id": "uni-001",
                "discipline_ids": ["disc-1", "disc-2"],
            },
            headers=superuser_headers,
        )
    ...
    async def test_set_disciplines_empty_list(self, client, superuser_headers):
        """设置空学科列表（清空关联）成功返回 204。"""
        self.mock_svc.set_disciplines.return_value = None

        resp = await client.post(
            "/admin/web-settings/universities/list/detail/disciplines",
            json={"university_id": "uni-001", "discipline_ids": []},
            headers=superuser_headers,
        )

        assert resp.status_code == 204

    async def test_set_disciplines_not_found(self, client, superuser_headers):
        """学科不存在返回 404。"""
        self.mock_svc.set_disciplines.side_effect = NotFoundException(
    ...
    async def test_set_disciplines_university_not_found(
        self, client, superuser_headers
    ):
        """院校不存在返回 404。"""
        self.mock_svc.set_disciplines.side_effect = NotFoundException(
    ...
    async def test_set_disciplines_missing_university_id(
        self, client, superuser_headers
    ):
        """缺少 university_id 返回 422。"""
        resp = await client.post(
            "/admin/web-settings/universities/list/detail/disciplines",
            json={"discipline_ids": ["disc-1"]},
            headers=superuser_headers,
        )

        assert resp.status_code == 422

    async def test_set_disciplines_missing_discipline_ids(
        self, client, superuser_headers
    ):
        """缺少 discipline_ids 返回 422。"""
        resp = await client.post(
            "/admin/web-settings/universities/list/detail/disciplines",
            json={"university_id": "uni-001"},
            headers=superuser_headers,
