...
from app.document import service as doc_service
from app.document.models import Document, DocumentCategory


DOC_REPO = "app.document.service.repository"
USER_REPO = "app.document.service.user_repo"
STORAGE = "app.document.service"
...
@pytest.mark.asyncio
@patch(f"{STORAGE}.save_file", new_callable=AsyncMock)
@patch(f"{STORAGE}.validate_storage_quota")
@patch(DOC_REPO)
@patch(USER_REPO)
...
async def test_upload_document_quota_exceeded(
    ...
    # 模拟配额校验抛出异常
    mock_validate_quota.side_effect = QuotaExceededException(
        message="存储配额不足"
    )
...
async def test_delete_document_success(
    mock_repo, mock_delete_file
):
    """删除自己的文档成功：删除文件和数据库记录。"""
    doc = _make_document(user_id="user-1")
    mock_repo.get_by_id = AsyncMock(return_value=doc)
