...
class TestPublicConfig:
    ...
    async def test_get_site_info(self, anon_client):
        """获取站点信息（公开接口，预置数据）。"""
        resp = await anon_client.get("/api/config/site_info")
...
class TestAdminConfig:
    ...
    async def test_update_config_and_revert(self, superuser_client):
        ...
        # 1. 获取原始值
        resp = await superuser_client.get(f"/api/config/{key}")
        assert resp.status_code == 200
        ...
        # 3. 验证更新生效
        verify_resp = await superuser_client.get(f"/api/config/{key}")
        assert verify_resp.status_code == 200
        assert verify_resp.json()["value"]["tagline"] == "e2e_test_tagline"

        # 4. 恢复原始值
        revert_resp = await superuser_client.put(
            f"/api/admin/config/{key}",
