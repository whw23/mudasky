...
vi.mock("@/components/public/UniversityList", () => ({
  UniversityList: ({ editable }: any) => (
    <div data-testid="university-list" data-editable={editable} />
  ),
}))
...
vi.mock("@/components/admin/web-settings/UniversityEditDialog", () => ({
  UniversityEditDialog: () => <div data-testid="university-edit-dialog" />,
}))

vi.mock("@/components/admin/web-settings/DisciplineManageDialog", () => ({
  DisciplineManageDialog: () => <div data-testid="discipline-manage-dialog" />,
}))
...
import { UniversityListBlock } from "@/components/blocks/UniversityListBlock"
...
describe("UniversityListBlock", () => {
  it("渲染 UniversityList 组件", () => {
    render(<UniversityListBlock block={makeBlock()} header={null} bg="" />)

    expect(screen.getByTestId("university-list")).toBeInTheDocument()
  })
  ...
  it("editable 模式渲染管理工具栏", () => {
    const onEdit = vi.fn()
    render(
      <UniversityListBlock block={makeBlock()} header={null} bg="" editable onEdit={onEdit} />,
    )
  ...
  it("editable 但无 onEdit 时不包裹 EditableOverlay", () => {
    render(
      <UniversityListBlock block={makeBlock()} header={null} bg="" editable />,
    )
  ...
  it("应用灰色背景样式", () => {
    const { container } = render(
      <UniversityListBlock block={makeBlock()} header={null} bg="bg-gray-50" />,
    )
