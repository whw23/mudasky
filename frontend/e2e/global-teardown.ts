...
async function globalTeardown(_config: FullConfig) {
  ...
  for (const { listPath, deletePath, nameField, idField } of cleanups) {
    try {
      const res = await page.request.get(
        `http://localhost/api/admin/${listPath}`,
        { headers },
      )
      if (res.ok()) {
        ...
        for (const item of items) {
          ...
          if (name?.startsWith("E2E")) {
            await page.request.post(
              `http://localhost/api/admin/${deletePath}`,
  ...
  try {
    const res = await page.request.get(
      "http://localhost/api/admin/users/list?keyword=E2E",
      { headers },
    )
    if (res.ok()) {
      const data = await res.json()
      ...
      for (const user of items) {
        if (user.username?.startsWith("E2E")) {
          await page.request.post(
            "http://localhost/api/admin/users/list/detail/delete",
