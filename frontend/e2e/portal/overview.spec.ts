...
test.describe("用户概览", () => {
  test("概览页面加载显示统计卡片", async ({ adminPage }) => {
    ...
    await expect(adminPage.getByText("文档数量")).toBeVisible({ timeout: 10_000 })
    await expect(adminPage.getByText("存储空间")).toBeVisible()
  })

  test("快捷操作链接可见", async ({ adminPage }) => {
    await gotoAdmin(adminPage, "/portal/overview")
    await expect(adminPage.getByText("上传文档")).toBeVisible({ timeout: 10_000 })
  ...
  test("编辑资料链接跳转", async ({ adminPage }) => {
    ...
    await adminPage.waitForURL(/\/portal\/profile/, { timeout: 15_000 })
