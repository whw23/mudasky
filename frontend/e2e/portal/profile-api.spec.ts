...
test.describe("个人资料端点覆盖", () => {
  ...
  test("密码修改区域可见", async ({ adminPage }) => {
    await gotoAdmin(adminPage, "/portal/profile")
    await expect(adminPage.getByText("修改密码")).toBeVisible({ timeout: 10_000 })
  })

  test("两步验证区域可见", async ({ adminPage }) => {
    await gotoAdmin(adminPage, "/portal/profile")
    await expect(adminPage.getByText("两步验证").first()).toBeVisible({ timeout: 10_000 })
  })

  test("登录设备区域可见", async ({ adminPage }) => {
    await gotoAdmin(adminPage, "/portal/profile")
    await expect(adminPage.getByText("登录设备")).toBeVisible({ timeout: 10_000 })
    await expect(adminPage.getByText("当前")).toBeVisible()
  })

  test("所属角色区域可见", async ({ adminPage }) => {
    await gotoAdmin(adminPage, "/portal/profile")
    await expect(adminPage.getByText("所属角色")).toBeVisible({ timeout: 10_000 })
  })
})
