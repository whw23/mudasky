...
10. 响应体保持不变（前端需要 user 信息存 localStorage）
```
...
| Cookie | Path | HttpOnly | SameSite | Secure | max-age |
|--------|------|----------|----------|--------|---------|
| access_token | / | true | Strict | 生产 true / 开发 false | 15*60 |
| refresh_token | /api/auth/refresh | true | Strict | 生产 true / 开发 false | 取决于 X-Keep-Login |
