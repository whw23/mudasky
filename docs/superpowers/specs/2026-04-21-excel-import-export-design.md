...
## 九、Playwright MCP 浏览器验证

每个阶段实施完成后，用 Playwright MCP 在浏览器中验证完整流程。

### 9.1 学科分类验证

1. 导航到网页设置 → 院校选择 → 学科分类管理
2. 点击"下载模板" → 确认文件下载成功
3. 点击"导入" → 上传填好的 Excel → preview 弹窗展示解析结果
4. 确认导入 → 页面刷新，新学科出现在列表中
5. 修改 Excel 部分字段重新导入 → 确认 merge：只更新了变更字段，未变更字段保留
6. 点击"导出" → 下载 ZIP → 确认包含完整数据

### 9.2 院校验证

1. 导航到网页设置 → 院校选择 → 院校管理
2. 下载模板 → 填写院校 + 专业（关联学科） + 放入 logo 图片 → 打包 ZIP
3. 导入 → preview 显示新增/更新/冲突
4. 处理冲突（学科不存在 → 选择创建新的）
5. 确认导入 → 院校出现在列表，专业正确关联学科，logo 图片显示
6. 导出 → 下载 ZIP → 确认包含 Excel + images/ 图片文件
7. 往返验证：导出的 ZIP 直接重新导入 → 所有记录标记为"无变化"

### 9.3 成功案例验证

1. 导航到网页设置 → 成功案例
2. 下载模板 → 填写案例 + 放入头像/offer 图片 → 打包 ZIP
3. 导入 → preview 显示结果，院校名匹配验证
4. 确认导入 → 案例出现在列表，图片显示正确
5. 导出 → 验证 ZIP 包含 Excel + 图片

### 9.4 文章验证

1. 导航到网页设置 → 任意分类页面（如出国留学）
2. 下载模板 → 填写文章元数据 + 放入 HTML/PDF 正文 → 打包 ZIP
3. 导入 → preview 显示结果
4. 确认导入 → 文章出现在当前分类列表
5. 切换到另一个分类页面 → 确认该分类下没有刚导入的文章（分类隔离）
6. 导出 → 验证只包含当前分类的文章

### 9.5 merge 验证（跨数据类型通用）

1. 导出已有数据的 ZIP
2. 修改 Excel 中部分字段，留空其他字段
3. 重新导入 → preview 标记为"更新(N 字段变更)"
4. 确认导入 → 只更新了变更字段，未变更字段保留原值
5. 再次导出 → 对比确认数据正确
...
| 2 | 通用工具 + 学科分类导入导出（最简单，验证整体流程）+ Playwright 验证 |
| 3 | 院校导入导出（最复杂，多 sheet + 图片 + 专业关联）+ Playwright 验证 |
| 4 | 成功案例导入导出 + Playwright 验证 |
| 5 | 文章导入导出 + Playwright 验证 |
| 6 | 前端 UI（工具栏 + 预览弹窗）+ 全量 Playwright 验证 |
