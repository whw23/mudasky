...
├── src/
│   └── app/
│       ├── core/                          # 通用共享层（API 和 Worker 共用）
│       │   ├── config.py                  # Pydantic Settings，环境变量管理
│       │   ├── database.py                # async engine / session factory
│       │   ├── security.py                # JWT 生成（HMAC-SHA256）
...
│       │
│       ├── auth/                          # 认证领域（共享）
│       │   ├── router.py                  # POST /api/auth/register, POST /api/auth/login, POST /api/auth/sms-code, POST /api/auth/refresh
│       │   ├── schemas.py                 # RegisterRequest, LoginRequest, SmsCodeRequest, TokenResponse
...
│       │
│       ├── user/                          # 用户领域（共享）
│       │   ├── router.py                  # GET/PATCH /api/users/me, PUT /api/users/me/password, PUT /api/users/me/phone, POST /api/users/me/2fa/enable, POST /api/users/me/2fa/disable
...
│       │
│       ├── api/                           # ── API 入口 ──
│       │   └── main.py                    # FastAPI 应用，挂载路由，注册异常处理
│       │
│       └── worker/                        # ── Worker 入口 ──
│           ├── main.py                    # 队列消费循环
...
| 入口 | 文件 | 启动命令 | Docker 服务 |
|------|------|---------|------------|
| API | `app/api/main.py` | `uvicorn app.api.main:app` | backend |
| Worker | `app/worker/main.py` | `python -m app.worker.main` | worker |

两者共享 `core/`、领域模块（models、repository、service），但有各自独立的入口和运行方式。
...
| worker | python:3.14 | Agent 工作流任务消费（后期启用，骨架先建） |
| db | PostgreSQL | 数据库 |
...
| `JWT_SECRET` | JWT 签名密钥（仅 Gateway 使用，Backend 不读取） |
| `ACCESS_TOKEN_EXPIRE_MINUTES` | Access token 过期时间（分钟，默认 15，Gateway 使用） |
| `REFRESH_TOKEN_EXPIRE_DAYS` | Refresh token 过期时间（天，默认 30，Gateway 使用） |
