...
mudasky 是一个教育机构的 Web 应用，第一期包含官网展示、用户注册/登录（含二步验证）、用户管理、内容管理（文章发布/审核）、文档上传管理、后台管理六个核心功能。后期将加入 Agent 工作流（多步骤流水线）处理文档。
...
项目采用 **三包分离架构**，全部在 `backend/` 下：`shared`（共享库）、`api`（API 服务）、`worker`（Worker 服务），通过 uv workspace 管理。

```text
backend/
├── pyproject.toml                         # uv workspace 根配置
├── shared/                                # ── 共享 Python 包 ──
│   ├── pyproject.toml                     # 包声明，被 api 和 worker 依赖
│   └── src/
│       └── app/
│           ├── core/                      # 通用共享层
...
│           │   ├── dependencies.py        # API 依赖注入（get_db, get_current_user）
│           │   ├── exceptions.py          # 自定义异常 + 全局异常处理
│           │   ├── pagination.py          # 通用分页模型
│           │   └── logging.py             # JSON 结构化日志配置
│           │
│           ├── auth/                      # 认证领域
│           │   ├── router.py              # POST /api/auth/register, /login, /sms-code, /refresh
│           │   ├── schemas.py             # RegisterRequest, LoginRequest, SmsCodeRequest, AuthResponse
│           │   ├── service.py             # 注册、三种登录、二步验证、续签
│           │   ├── repository.py          # 验证码、refresh token 查询/存储
│           │   ├── models.py              # SmsCode, RefreshToken 模型
│           │   └── sms.py                 # 阿里云短信 SDK 封装
│           │
│           ├── user/                      # 用户领域
│           │   ├── router.py              # GET/PATCH /me, PUT /me/password, /me/phone, POST /me/2fa/*
│           │   ├── schemas.py             # UserCreate, UserResponse, UserUpdate, PasswordChange 等
│           │   ├── service.py             # 用户 CRUD、密码、手机号、2FA 管理
│           │   ├── repository.py          # 用户查询封装
│           │   └── models.py              # User ORM 模型
│           │
│           ├── document/                  # 文档领域
│           │   ├── router.py              # POST /documents, GET /documents/{id}, DELETE
...
│           │
│           ├── content/                   # 内容管理领域
│           │   ├── router.py              # GET /content/articles, POST /content
...
│           │   └── service.py
│           │
│           └── worker/                    # Worker 共享模块（API 入队、Worker 消费都需要）
│               ├── models.py              # Task ORM 模型
│               └── queue.py               # PostgreSQL 任务队列（入队/抢占/完成）
│
├── api/                                   # ── API 服务（独立容器） ──
...
│   ├── scripts/
│   │   └── start.sh                       # 容器启动脚本（迁移 + uvicorn）
│   ├── alembic/                           # 数据库迁移（只有 API 容器执行）
│   │   ├── versions/
│   │   └── env.py
│   ├── alembic.ini
│   └── tests/
...
| 包 | 路径 | 职责 | 依赖 | Docker 服务 |
|---|------|------|------|------------|
| shared | `backend/shared/` | 共享库：core + 所有领域模块 + Worker 模型/队列 | 第三方库 | — |
| api | `backend/api/` | FastAPI 入口 + Alembic 迁移 | shared | api |
| worker | `backend/worker/` | Worker 消费循环入口 | shared | worker |

### 1.5 关键设计决策

**auth 领域有独立的 models 和 repository**
`auth/models.py` 包含 SmsCode（短信验证码）和 RefreshToken（refresh token 轮换）两个模型，`auth/repository.py` 封装验证码和 token 的数据访问。认证同时依赖 `user.repository` 查询和创建用户。
...
| id | UUID | 主键 |
| phone | str | 手机号（唯一） |
| username | str | 用户名（唯一，nullable，注册后可设置） |
...
- **勾选**：refresh token Cookie 设置 `max-age=30天`，关闭浏览器后仍保持登录
- **不勾选**：refresh token Cookie 不设 `max-age`（会话级），关闭浏览器后自动登出
- access token 始终为 15 分钟，不受此选项影响
- 前端通过请求头 `X-Keep-Login: true/false` 传递，OpenResty 读取后决定 Cookie 的 `max-age`
...
**Backend 不接触 JWT_SECRET，不写 Cookie。** 所有 JWT 生成和 Cookie 操作由 OpenResty 完成。

| 操作 | Backend 职责 | OpenResty 职责 |
|------|-------------|---------------|
...
2. 用户输入验证码 + 可选密码 → 前端调 POST /api/auth/register
3. OpenResty 透传请求到后端
4. 后端校验验证码 → 创建用户（密码可选，有则哈希存储） → 返回用户信息
5. OpenResty 拦截响应 → 生成 JWT → 添加 Set-Cookie
6. 前端将用户信息存入 localStorage
```

**登录流程（三种方式）：**

```text
方式一：手机号 + 验证码（直接登录，不受二步验证影响）
1. 用户输入手机号 → 前端调 POST /api/auth/sms-code
2. 用户输入验证码 → 前端调 POST /api/auth/login（body: {phone, code}）

方式二：用户名 + 密码
1. 用户输入用户名和密码 → 前端调 POST /api/auth/login（body: {username, password}）

方式三：手机号 + 密码
1. 用户输入手机号和密码 → 前端调 POST /api/auth/login（body: {phone, password}）

方式二和方式三在开启二步验证时：
1. 后端校验密码 → 返回 {"step": "2fa_required"}
2. 用户选择验证方式：短信验证码 或 Authenticator（TOTP）
3. 前端调 POST /api/auth/login（原参数 + code 或 totp）
4. 后端校验二步验证码 → 返回用户信息

后续流程相同：
3. OpenResty 透传请求到后端
4. 后端校验验证码或密码 → 返回用户信息（JSON，不含 token，不设 Cookie）
5. OpenResty 拦截响应 → 用用户信息生成 access_token + refresh_token → 添加 Set-Cookie
6. 前端将用户信息存入 localStorage
```
...
2. 自动调 POST /api/auth/refresh
3. OpenResty 从 Cookie 读 refresh_token → 转发给后端（通过请求体或请求头）
4. 后端验证 refresh token 哈希 → 轮换 → 查库确认用户状态 → 返回用户信息（JSON，不设 Cookie）
5. OpenResty 拦截响应 → 生成新 access_token + refresh_token → 添加 Set-Cookie
6. 前端更新 localStorage 中的用户信息
7. 自动重试原请求
```
...
| 职责 | 说明 |
|------|------|
| 反向代理 | `/api/*` → backend:8000，`/*` → frontend:3000 |
...
| 服务 | 镜像 | 作用 |
|------|------|------|
| gateway | OpenResty | 反向代理、认证、限流 |
| frontend | Node 22 | Next.js 服务（SSG/ISR/CSR 混合渲染），常驻运行 |
| api | python:3.14 | FastAPI 业务逻辑（依赖 backend 包） |
| worker | python:3.14 | Agent 工作流任务消费（依赖 backend 包，后期启用） |
| db | PostgreSQL | 数据库 |
...
| api | 8000 → 宿主机 | 直接调试 API |
| db | 5432 → 宿主机 | 数据库客户端连接 |
| frontend | 3000 → 宿主机 | Next.js dev server |
...
| 用户上传文件 | named volume（挂载到 api 容器） |

### 4.4 开发环境特性

- 后端挂载源码目录，支持热重载
- 前端使用 Next.js dev server 运行，gateway 反代到 `frontend:3000`
...
```text
docker/
├── api.Dockerfile             # python:3.14，安装 backend + api 包
├── worker.Dockerfile          # python:3.14，安装 backend + worker 包
├── frontend.Dockerfile        # Node 22，Next.js 服务，常驻运行
└── gateway.Dockerfile         # OpenResty 官方镜像
```

### 4.6 前端部署

Next.js 作为 Node 服务常驻运行（端口 3000），Gateway 反代所有非 `/api/*` 请求到 frontend 服务。支持 SSG/ISR/CSR 混合渲染：

| 页面 | 渲染方式 | 说明 |
|------|---------|------|
| 首页、关于我们、联系我们 | SSG | 构建时预渲染，极快 |
| 文章列表、文章详情 | ISR | 首次请求时生成，之后缓存，定期重新验证 |
| 登录、用户中心、后台管理 | CSR | 纯客户端渲染，不需要 SEO |
...
各服务使用独立的环境变量文件，遵循最小权限原则：

```text
env/
├── gateway.env       # JWT_SECRET, ACCESS_TOKEN_EXPIRE_MINUTES, REFRESH_TOKEN_EXPIRE_DAYS
├── backend.env       # DB_*, SMS_*, MAX_UPLOAD_SIZE_MB, DEFAULT_STORAGE_QUOTA_MB
├── worker.env        # DB_*
└── db.env            # POSTGRES_DB, POSTGRES_USER, POSTGRES_PASSWORD
```

Docker Compose 中按服务指定 `env_file`，禁止所有服务共用同一个 `.env`。

各服务所需变量：

| 服务 | 变量 |
|------|------|
| gateway | `JWT_SECRET`、`ACCESS_TOKEN_EXPIRE_MINUTES`（默认 15）、`REFRESH_TOKEN_EXPIRE_DAYS`（默认 30） |
| backend | `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD`、`SMS_ACCESS_KEY_ID`/`SMS_ACCESS_KEY_SECRET`/`SMS_SIGN_NAME`/`SMS_TEMPLATE_CODE`、`MAX_UPLOAD_SIZE_MB`、`DEFAULT_STORAGE_QUOTA_MB` |
| worker | `DB_HOST`/`DB_PORT`/`DB_NAME`/`DB_USER`/`DB_PASSWORD` |
...
后期在后端增加 `workflow/` 领域模块，实现多步骤文档处理流水线。

- 架构：任务队列 + Worker 容器，FIFO 先入先出抢占模式
- 每个 Worker 独立容器，容器内单线程，通过增加容器数量（`docker compose up --scale worker=N`）实现并发
- 用户提交文档 → 任务入队 → Worker 抢占消费 → 多步骤 Agent 处理 → 结果写回数据库
- 任务队列基于 PostgreSQL 实现（如 `SELECT ... FOR UPDATE SKIP LOCKED`），不引入额外中间件
- 具体实现方式和 LLM 服务商待后续设计
