...
8. 后台预览与公开页面不一致：编辑装饰（虚线边框、按钮、添加卡片）始终可见，改变了布局和视觉
...
通过 Playwright 对比公开页面（`/about`）和后台预览（`/admin/web-settings`）的 ContactInfo 区域，发现以下不一致问题。本约束适用于所有 Block 类型。

### 发现的问题

| 差异点 | 后台预览 | 公开页面 | 影响 |
|-------|---------|---------|------|
| 编辑装饰 | 蓝色虚线边框 + 删除/编辑按钮始终可见 | 无 | 卡片视觉不一致 |
| 添加条目卡片 | 占一个网格位 | 不存在 | 网格列数/换行不同 |
| Block 外层框 | SpotlightOverlay 虚线大框 | 无 | 区域边距不同 |
| Block 工具栏 | 顶部 contact_info 标签栏 | 无 | 占据垂直空间 |
| 卡片边框尺寸 | border 导致布局偏移 | 无边框 | 卡片尺寸不同 |

### 一致性规则（所有 Block 通用）

1. **编辑装饰仅 hover 显示**：虚线边框、删除按钮、编辑铅笔在非 hover 状态下完全不可见，不占据空间
2. **添加按钮不影响网格**：添加按钮仅在 hover Block 时显示，或使用绝对定位，不占网格位
3. **使用 outline 而非 border**：编辑装饰使用 CSS `outline` 或 `box-shadow`，避免改变元素尺寸和布局
4. **Block 工具栏绝对定位**：已有的 SpotlightOverlay 工具栏使用绝对定位浮于内容之上，不挤占内容空间
5. **验证流程**：改造完成后必须用 Playwright 对比公开页面和后台预览的截图，确认非 hover 状态下一致
